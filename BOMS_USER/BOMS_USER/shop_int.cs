...
namespace BOMS_USER
{
    public partial class shop_int : Form
    {
       ...
       (
        ...
        private void shop_int_Load(object sender, EventArgs e)
        {
            btn_sizeG.Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, btn_sizeG.Width, btn_sizeG.Height, 40, 40));

            btn_About.Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, btn_About.Width, btn_About.Height, 40, 40));
        ...
        private void tbS_TextChanged(object sender, EventArgs e)
        {
                try
                {
                    using (OleDbConnection connection = new OleDbConnection(connectionString))
                    {
                        ...
                        // Combine search queries for both tables
                        string searchQuery = @"
                SELECT product_id AS ProductID, product_name AS ProductName, product_price AS ProductPrice, 
                       product_image AS ProductImage, stock_status AS StockStatus 
                FROM products 
                WHERE product_name LIKE @searchText
                UNION ALL
                SELECT Product_ID AS ProductID, Product_name AS ProductName, Price_Size_VS AS ProductPrice, 
                       Picture AS ProductImage, stock_status AS StockStatus 
                FROM Products_Size 
                WHERE Product_name LIKE @searchText";
                        ...
                        using (OleDbCommand command = new OleDbCommand(searchQuery, connection))
                        {
                            ...
                            using (OleDbDataReader reader = command.ExecuteReader())
                            {
                                ...
                                {
                                    int productId = reader.GetInt32(0);
                                    string productName = reader["ProductName"]?.ToString() ?? "Unnamed Product";
                                    decimal productPrice = Convert.ToDecimal(reader["ProductPrice"]);
                                    string productImagePath = reader["ProductImage"]?.ToString();
